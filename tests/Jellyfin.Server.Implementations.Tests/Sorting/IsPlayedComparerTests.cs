...
public sealed class IsPlayedComparerTests : IDisposable
{
    ...
    public void Compare_NullItem_HandlesGracefully()
    {
        ...
        var cmp = new IsPlayedComparer { User = user };
    ...
    {
        var cmp = new IsPlayedComparer();

        Assert.Throws<ArgumentNullException>(() => cmp.Compare(new Movie(), new Movie()));
    }

    [Fact]
    public void Compare_PlayedVsUnplayed_PlayedSortsFirst()
    {
        var user = CreateUser();
